...
public class MemberRegService {
	...
	public int memberReg(MemberRegRequest regRequest, HttpServletRequest request) {
		...
		try {
			...
			// 회원 간에 저장되는 파일 이름이 같으면 덮어쓴다 -> 새로운 회원별 고유한 파일 이름을 만들자!!(회원 아이디를 이용할 예정), 새로운 파일이름에 확장자 추가
			String newFileName = regRequest.getMemberid() //회원 아이디 가져오기
					+ System.currentTimeMillis(); // <- 이 메소드가 반환하는 것이 long 타입의 정수형이긴 해도 앞에서 더할 값인 regRequest.getMemberid()가 문자열이기 때문에 결과적으로 문자열간의 결합이 되게 된다.
					//+ chkFileType(regRequest.getPhoto());
			// cool123128936798123987

			//파일 저장하기 - 그러기 위해서는 새로운 File 객체 필요
			newFile = new File(newDir, newFileName); //파일을 업로드 해서 DB에 저장할 때 DB와 연결하는 과정에서 예외가 발생할 떄 newFile이 존재하는지 여부를 체크한 다음 있다면 지워주는 작업을 해주면 된다. 그래서 newFile에 대한 객체를 위에다 선언함.
			//파일 저장하기
			if(regRequest.getPhoto() != null && !regRequest.getPhoto().isEmpty()) {
...
//			}
			...
			// Member 객체 생성 해주고,  저장된 파일의 이름을 set하도록 한다.(RegRequest의 toMember 메소드는 파일 이름을 원래 이름 그대로 저장하려고 하기 때문에 여기서 파일 이름을 바꿔주는 처리를 해주어야 한다.
			...
			// DB 예외 발생 시 -> 저장된 파일을 삭제 ( DB와 연결하는 과정에서 예외가 발생할 떄 newFile이 존재하는지 확인하고 , 있다면 파일을 지워주고 없던일처럼 만들기)
