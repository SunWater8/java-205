...
//배열에 있는 요소를 table의 tr 행으로 만들어서 출력할 수 있도록 하기

function setList() {
    // table의 tbody를 캐스팅하기
    var list = document.querySelector('#list');
    var tbody = list.innerHTML; //tbody의 list를 html에서 가져오기
    //가져온 배열을 반복시켜주기

    if (members.length < 1) { //배열에 데이터가 존재하지 않을 때
        tbody += '<tr>';
        tbody += '<td colspan="5">데이터가 존재하지 않습니다.</td>'; //colspan으로 5칼럼을 병합해주기
        tbody += '</tr>';
    } else { //배열에 데이터가 존재할 때.
