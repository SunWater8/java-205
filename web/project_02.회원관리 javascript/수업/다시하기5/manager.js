...
//---------------------------------------------------------------------------------------------
/* 배열을 JSON(문자열)로 하고 localstorage에 저장해주기
저장
수정
삭제
...
window.onload = function () {
    ...
    // ----------------------------------------------------------localstorage--------------
    // localStorage 에 저장된 데이터가 있는지 확인

    //localStorage.getItem('members')가 없으면 null을 반환
    if(localStorage.getItem('members') == null){
        //members배열을 저장하기 (객체를 문자열로 바꾸고 저장)
        localStorage.setItem('members', JSON.stringify(members));
    }else{
        members = JSON.parse(localStorage.getItem('members')); //JSON 문자열 ->객체로 변환해 줘야 함.
        //members로 할당 하기
        console.log(members);
...
//수정버튼 누르고 수정 창 닫게 하는 함수 만들기

function editMemberClass(){

    //editFormArea를 캐스팅 해서 안보이게 하기
    document.querySelector('#editFromArea').style.display='none';
}
