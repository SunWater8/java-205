...
--17. SUBSTR 함수를 사용하여 
--4월에 입사한 사원을 출력하시오.
select ename, hiredate, substr(hiredate,4,2) as "hire in April"
from emp
where substr(hiredate,4,2) = '04' --숫자 4라고 해도 됨. 작은 따움표 쓰지 않아도 자동 형변환 되기 때문
;
...
select ename, to_char(hiredate, 'yy. mm. mon dy')
from emp;

--20. 올해 몇 일이 지났는지 출력하시오. 
-- 현재날짜에서 올해 1월 1일을 뺀 결과를 출력하고 
-- TO_DATE 함수를 사용하여 데이터 형을 일치 시키시오.
select sysdate,to_date('2021.01.01', 'yyyy.mm.dd') as "dates from 2021.01.01 to now"
from dual
;
...
select  max(sal)as "급여 최고액" , min(sal)as"급여 최저액", sum(sal)as"급여 총액", round(avg(sal),0) as"급여 평균액"
from emp
;
...
--26. 관리자 수를 출력하시오.
select count(distinct mgr) --count 함수에는 null 값을 포함하지 않는다.
from emp;

--27. 급여 최고액, 급여 최저액의 차액을 출력하시오.
select max(sal), min(sal), max(sal)-min(sal)as "max-min"
from emp;

--28. 직급별 사원의 최저 급여를 출력하시오.           group by job
--관리자를 알 수 없는 사원과                        where mgr is not null
--최저 급여가 2000 미만인 그룹은 제외시키고           having min(sal) >= 2000
--결과를 급여에 대한 내림차순으로 정렬하여 출력하시오.   order by min(sal) desc
...
select deptno, 
       ...
       decode(deptno,10, 'NEW YORK',
                     20, 'DALLAS',
                     30, 'CHICAGO') as local,
       count(*), round(avg(sal))as"AVG SAL"
from emp
group by deptno;
